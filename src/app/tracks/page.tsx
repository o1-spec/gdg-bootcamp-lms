...
export default function MyTracksPage() {
  ...
  const detailedTrackList = Object.values(mockDetailedTracks);

  const filteredTracks = detailedTrackList.filter((track) => {
    return (
      searchQuery === '' ||
            ...
            <div className="flex items-center gap-3">
              <div className="rounded-2xl border border-[#E5DFD0] bg-white px-4 py-2.5 shadow-2xs">
                ...
                <span className="text-lg font-black text-[#0D0E11]">
                  {detailedTrackList.length} Tracks
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {filteredTracks.map((track, idx) => (
              <EnrolledTrackCard
                key={track.id}
                track={track}
                // Controlled visual variety: middle card light, outer dark or vice versa
