...
export function StatsCard({
  ...
  const content = (
    <div className="relative overflow-hidden rounded-3xl border border-gdg-border bg-white p-6 shadow-xs hover:shadow-md transition-all duration-200">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-2">
            ...
            {badge && (
              <span
                className={cn(
                  ...
                  badge.variant === 'positive' && 'bg-gdg-green/15 text-gdg-green-dark',
                  badge.variant === 'urgent' && 'bg-gdg-red/15 text-gdg-red',
                  badge.variant === 'neutral' && 'bg-gdg-border text-gdg-black'
                )}
              >
                {badge.text}
              </span>
          ...
          {subtext && (
            <p className="text-xs font-medium text-gdg-gray">{subtext}</p>
          )}
        </div>
