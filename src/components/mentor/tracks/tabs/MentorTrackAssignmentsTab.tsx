  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-black text-gdg-black">Track Assignments</h2>
          <p className="text-xs text-gdg-gray">Assessments and projects assigned to this track cohort.</p>
        </div>
        <Link
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {assignments.map((assignment) => {
            ...
            return (
              <div
                key={assignment.id}
                className="p-6 rounded-3xl bg-white border border-gdg-border shadow-sm hover:border-gdg-black transition-all space-y-4"
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h3 className="text-base font-black text-gdg-black mt-1">{assignment.title}</h3>
                  </div>
                  <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-gdg-cream text-gdg-gray border border-gdg-border">
                    Due {new Date(assignment.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                  </span>
                </div>

                <p className="text-xs text-gdg-gray line-clamp-2">{assignment.description}</p>
                ...
                <div className="pt-4 border-t border-gdg-border flex items-center justify-between">
                  ...
                  <Link
                    ...
                    className="text-xs font-bold text-gdg-blue hover:underline"
                  >
                    Review Submissions →
                  </Link>
                </div>
