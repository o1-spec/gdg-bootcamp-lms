...
export function MentorAnnouncementsClient({
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case 'URGENT':
        return (
          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-[#EA4335] text-white">
            Urgent
          </span>
        );
      case 'HIGH':
        return (
          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-[#FBBC04] text-[#0D0E11]">
            High Priority
          </span>
        );
      case 'LOW':
        return (
          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-[#FAF7EE] text-[#5F6368] border border-[#E5DFD0]">
            Notice
          </span>
        );
      default:
        return (
          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-[#4285F4]/10 text-[#4285F4]">
            General Update
          </span>
        );
    }
        ...
        <main className="flex-1 p-6 sm:p-8 lg:p-10 space-y-8 max-w-7xl w-full mx-auto">
          ...
          {filteredAnnouncements.length === 0 ? (
            <div className="rounded-3xl bg-white border border-[#E5DFD0] p-12 text-center shadow-sm max-w-md mx-auto space-y-3">
              ...
              <h3 className="text-lg font-black text-[#0D0E11]">No Announcements Found</h3>
            ...
            <div className="space-y-4">
              {filteredAnnouncements.map((ann) => (
                <div
                  key={ann.id}
                  className="p-6 sm:p-8 rounded-3xl bg-white border border-[#E5DFD0] hover:border-[#0D0E11] transition-all shadow-sm space-y-4"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#E5DFD0] pb-4">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {ann.track ? (
                        ...
                        <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-[#4285F4]/10 text-[#4285F4] border border-[#4285F4]/30">
                          Cohort-Wide Global
                        </span>
                      )}
                  ...
                  <div>
                    <h3 className="text-xl font-black text-[#0D0E11]">{ann.title}</h3>
                    <p className="text-sm text-[#5F6368] mt-2 whitespace-pre-wrap leading-relaxed font-medium">
                      {ann.content}
                    </p>
                  </div>
                </div>
