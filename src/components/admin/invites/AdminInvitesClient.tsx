...
export function AdminInvitesClient({
  ...
  return (
    <div className="min-h-screen bg-gdg-black text-gdg-cream flex antialiased">
      {/* Sidebar */}
      <AdminSidebar
        currentTab="invites"
        admin={admin}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <AdminHeader
          ...
          actions={
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-gdg-blue hover:bg-gdg-blue/90 text-xs font-bold text-white shadow-sm transition-all cursor-pointer"
            >
              <Plus className="w-4 h-4 text-gdg-yellow" />
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl w-full mx-auto">
          {/* Toast Notification */}
          {toastMessage && (
            <div className="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-3 rounded-2xl bg-white text-gdg-black text-xs font-bold shadow-2xl border border-white/20 animate-in fade-in slide-in-from-bottom-2">
              <CheckCircle2 className="w-4 h-4 text-gdg-green" />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="p-4 rounded-3xl bg-[#14151B] border border-white/10 space-y-1">
              <div className="text-[11px] font-bold uppercase tracking-wider text-gdg-green">Active Invites</div>
              <div className="text-2xl font-black text-gdg-green">{activeCount}</div>
            </div>
            <div className="p-4 rounded-3xl bg-[#14151B] border border-white/10 space-y-1">
              <div className="text-[11px] font-bold uppercase tracking-wider text-gdg-blue">Total Redemptions</div>
              <div className="text-2xl font-black text-gdg-blue">{totalUses}</div>
            </div>
            <div className="p-4 rounded-3xl bg-[#14151B] border border-white/10 space-y-1">
              <div className="text-[11px] font-bold uppercase tracking-wider text-gdg-yellow">Avg. Uses / Code</div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 p-4 rounded-3xl bg-[#14151B] border border-white/10">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 rounded-2xl bg-white/5 border border-white/10 text-xs text-white placeholder:text-white/40 focus:outline-none focus:border-gdg-blue"
              />
            </div>
          ...
          {displayedInvites.length > 0 ? (
                    ...
                    <div key={inv.id} className="p-4 space-y-3">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1.5">
                          ...
                          >
                            {copiedCode === inv.code ? (
                              <Check className="w-3.5 h-3.5 text-gdg-green" />
                            ) : (
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg text-white/50 hover:text-gdg-blue hover:bg-white/10 transition-colors cursor-pointer"
                          >
                        ...
                        {!inv.isActive ? (
                          ...
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-gdg-red/15 text-gdg-red border border-gdg-red/30">
                            Expired
                          </span>
                        ) : isFull ? (
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-gdg-yellow/15 text-gdg-yellow border border-gdg-yellow/30">
                            Full
                          </span>
                        ) : (
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-gdg-green/15 text-gdg-green border border-gdg-green/30">
                            Active
                          </span>
                        )}
                      </div>
                      ...
                      <div className="p-3 rounded-2xl bg-white/3 border border-white/5 space-y-1.5 text-xs">
                        ...
                        <div className="pt-1">
                          {inv.allowTrackSelection ? (
                            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold bg-gdg-yellow/15 text-gdg-yellow border border-gdg-yellow/30">
                              <Sparkles className="w-3 h-3" />
                              <span>Multi-Track (Max {inv.maxTrackSelections || 1})</span>
                            </span>
                          ) : inv.trackName ? (
                            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold bg-gdg-blue/15 text-gdg-blue border border-gdg-blue/30">
                      ...
                      <div className="flex items-center justify-between gap-2 text-xs pt-1 border-t border-white/5 text-[11px]">
                        ...
                        <div>
                          ...
                          <span className={`font-mono ${isExpired ? 'text-gdg-red font-bold' : 'text-white/60'}`}>
                      ...
                      <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                        ...
                        >
                          {inv.isActive ? (
                            <>
                              <ToggleRight className="w-3.5 h-3.5 text-gdg-green" />
                    ...
                    {displayedInvites.map((inv) => {
                      ...
                      return (
                        <tr key={inv.id} className="hover:bg-white/2 transition-colors">
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => handleCopyLink(inv.code)}
                                title="Copy Shareable Join Link"
                                className="p-1.5 rounded-lg text-white/40 hover:text-gdg-blue hover:bg-white/10 transition-colors cursor-pointer"
                              >
                                <ExternalLink className="w-3.5 h-3.5" />
                              </button>
                            </div>
                          ...
                          <td className="px-5 py-4">
                            {inv.allowTrackSelection ? (
                              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-gdg-yellow/15 text-gdg-yellow border border-gdg-yellow/30">
                                <Sparkles className="w-3 h-3" />
                                <span>Multi-Track (Max {inv.maxTrackSelections || 1})</span>
                              </span>
                            ) : inv.trackName ? (
                              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-gdg-blue/15 text-gdg-blue border border-gdg-blue/30">
                                <Layers className="w-3 h-3" />
                                <span>{inv.trackName}</span>
                              </span>
                            ) : (
                          ...
                          <td className="px-5 py-4">
                            {inv.expiresAt ? (
                              <span className={`text-[11px] font-medium ${isExpired ? 'text-gdg-red font-bold' : 'text-white/60'}`}>
                          ...
                          <td className="px-5 py-4">
                            {!inv.isActive ? (
                              ...
                              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-gdg-red/15 text-gdg-red border border-gdg-red/30">
                                Expired
                              </span>
                            ) : isFull ? (
                              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-gdg-yellow/15 text-gdg-yellow border border-gdg-yellow/30">
                                Limit Reached
                              </span>
                            ) : (
                              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-gdg-green/15 text-gdg-green border border-gdg-green/30">
                                Active
                              </span>
                            )}
                          </td>
                          ...
                          <td className="px-5 py-4 text-right">
                            <div className="flex items-center justify-end gap-1.5">
                              ...
                              >
                                {inv.isActive ? (
                                  <ToggleRight className="w-4 h-4 text-gdg-green" />
                                ) : (
            ...
            <div className="p-12 text-center rounded-3xl bg-[#14151B] border border-white/10 space-y-3">
              ...
              <button
                onClick={() => setIsCreateOpen(true)}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-gdg-blue text-xs font-bold text-white hover:bg-gdg-blue/90 transition-all cursor-pointer"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-lg rounded-3xl bg-[#14151B] border border-white/15 p-6 sm:p-8 shadow-2xl space-y-5 text-white max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4 text-xs">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-[11px] font-bold text-gdg-blue hover:underline"
                  >
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-2xl bg-white/5 border border-white/10 font-mono font-bold text-white uppercase focus:outline-none focus:border-gdg-blue"
                />
              </div>
              ...
              <div className="space-y-1.5">
                <label className="font-bold text-white/90">
                  Bootcamp <span className="text-gdg-red">*</span>
                </label>
                <select
                  required
                  value={formData.bootcampId}
                  onChange={(e) => setFormData({ ...formData, bootcampId: e.target.value, cohortId: '', trackId: '' })}
                  className="w-full px-4 py-2.5 rounded-2xl bg-white/5 border border-white/10 font-bold text-white focus:outline-none focus:border-gdg-blue"
                >
                  {bootcamps.map((b) => (
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 rounded-2xl bg-white/5 border border-white/10 font-medium text-white focus:outline-none focus:border-gdg-blue"
                >
              ...
              <div className="p-3.5 rounded-2xl bg-white/5 border border-white/10 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    className="w-4 h-4 rounded text-gdg-blue cursor-pointer"
                  />
                </div>
                ...
                {formData.allowTrackSelection ? (
                  <div className="space-y-1.5 pt-2 border-t border-white/10">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none focus:border-gdg-blue"
                    />
                  </div>
                ) : (
                  <div className="space-y-1.5 pt-2 border-t border-white/10">
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2 rounded-xl bg-white/5 border border-white/10 font-medium text-white focus:outline-none focus:border-gdg-blue"
                    >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-2xl bg-white/5 border border-white/10 text-white focus:outline-none focus:border-gdg-blue"
                  />
                </div>
