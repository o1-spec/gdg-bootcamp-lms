...
export function LessonItem({
  ...
  const getStatusIcon = () => {
    switch (lesson.status) {
      case 'completed':
        return <CheckCircle2 className="h-4 w-4 text-[#34A853]" />;
      case 'current':
        return <PlayCircle className="h-4 w-4 animate-pulse" style={{ color: accentColor }} />;
      case 'locked':
      default:
        return <Lock className="h-4 w-4 text-[#5F6368]" />;
    }
  };
  ...
  const getTypeIcon = () => {
    switch (lesson.type) {
      case 'exercise':
        return <FileCode2 className="h-3 w-3 text-[#5F6368]" />;
      case 'project':
        return <Award className="h-3 w-3 text-[#FBBC04]" />;
      case 'lecture':
      default:
        return <Video className="h-3 w-3 text-[#5F6368]" />;
    }
  };
    ...
    <Link
      ...
      className={cn(
        ...
        lesson.status === 'current'
          ? 'bg-white border-[#0D0E11] shadow-xs'
          : lesson.status === 'completed'
          ? 'bg-white/80 border-[#E5DFD0] hover:border-[#0D0E11]/30'
          : 'bg-[#FAF7EE]/60 border-[#E5DFD0]/60 opacity-80 hover:opacity-100 hover:border-[#E5DFD0]'
      )}
    >
      <div className="flex items-start sm:items-center gap-3">
        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#FAF7EE] border border-[#E5DFD0]">
          {getStatusIcon()}
        </div>
        ...
        <div className="space-y-0.5">
          <div className="flex items-center gap-2">
            ...
            {lesson.status === 'completed' && (
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#34A853]/15 text-[#1e7e34]">
                COMPLETED
              </span>
            )}
          </div>
          ...
          <h5
            className={cn(
              ...
              lesson.status === 'locked' ? 'text-[#5F6368]' : 'text-[#0D0E11]'
            )}
          >
            {lesson.title}
          </h5>
      ...
      <div className="flex items-center justify-between sm:justify-end gap-3 pl-10 sm:pl-0">
        <div className="flex items-center gap-2 text-[11px] font-medium text-[#5F6368]">
        ...
        <span
          className={cn(
            ...
            lesson.status === 'current'
              ? 'bg-[#0D0E11] text-[#FAF7EE] group-hover:bg-[#1a1b20]'
              : lesson.status === 'completed'
              ? 'border border-[#E5DFD0] text-[#0D0E11] group-hover:bg-[#FAF7EE]'
              : 'border border-transparent text-[#5F6368]'
          )}
        >
          {lesson.status === 'completed'
            ? 'Review'
